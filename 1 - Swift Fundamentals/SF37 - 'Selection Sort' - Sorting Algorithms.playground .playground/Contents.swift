...
func insertSort(numbers: [Int]) -> [Int]?{
    ...
    guard var minCount = numbers.min() else { return sortedNumbers}
    guard var maxCount = numbers.max() else { return sortedNumbers}
    while minCount <= maxCount {
        if numbers.contains(minCount) {
            sortedNumbers.append(minCount)
        }
        minCount += 1
    }
...
var numbers = [-43, 6, -2, 29, 16]
print(insertSort(numbers: numbers)!)
