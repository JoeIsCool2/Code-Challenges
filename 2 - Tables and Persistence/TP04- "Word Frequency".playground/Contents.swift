...
func wordCounting( sentence: String ) {
    let lowercased = sentence.lowercased()
    let allWords = lowercased.split(separator: " ")
    let set = NSCountedSet(array: allWords)
    for object in set {
        let count = set.count(for: object)
        print("\(object):\(count)")
    }
}

wordCounting(sentence: "Beware the Jabberwock my son The jaws that bite The claws that catch")
