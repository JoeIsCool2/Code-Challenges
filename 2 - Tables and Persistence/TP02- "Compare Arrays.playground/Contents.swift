...
func formatingPhoneNumber(phoneNumber: [Int]) -> String? {
    var phoneNumberCheck = phoneNumber
    phoneNumberCheck.filter { $0 < 10 }
    if phoneNumberCheck == phoneNumber && phoneNumber.count == 10 {
        let area = phoneNumber.prefix(3)
        let mid = phoneNumber.dropFirst(3).prefix(3)
...
print(formatingPhoneNumber(phoneNumber: [5,5,5,8,6,7,5,3,0,9])!)
