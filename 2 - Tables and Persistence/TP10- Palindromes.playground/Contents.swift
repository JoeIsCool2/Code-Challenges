...
func isPalindrome(word: String) -> Bool {
    let reversedText = word.lowercased().reversed()
    if word.lowercased() == String(reversedText)  {
        return true
    }
    return false
}

func isPalindromePhrase(sentace: String) -> Bool {
    let word = sentace.filter { $0 != " " && $0 != "." && $0 != "!" && $0 != "?" }
    let reversedText = word.lowercased().reversed()
    if word.lowercased() == String(reversedText)  {
        return true
    }
...
print(isPalindrome(word: "rotator"))
print(isPalindrome(word: "mississippi"))
print(isPalindromePhrase(sentace: "Too bad I hid a boot."))
